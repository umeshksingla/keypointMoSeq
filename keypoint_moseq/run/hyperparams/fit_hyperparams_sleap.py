...
def train(train_paths, project_dir, save_dir, use_instance):
    '''

    Parameters
    ----------
    train_paths
    project_dir: project directory to read config from
    save_dir: where to save the resulting pca.p, checkpoint files. Historically, it has been the project_dir for this
    purpose but for multiple runs, for example during cross validation, it's better to save artefacts separately
    belonging to different runs
    use_instance
    ...
    # Fit PCA
    run_fit_PCA(data, project_dir, save_dir)

    # Initialize and fit ARHMM
    _, _, name = fit_keypoint_ARHMM(project_dir, save_dir, data, batch_info)

    # Fit SLDS
    arhmm_checkpoint_path = os.path.join(save_dir, name, 'checkpoint.p')
    name = resume_slds_fitting_to_new_data(train_paths, arhmm_checkpoint_path, project_dir, save_dir)
...
def fitCV(sleap_paths, project_dir, use_instance):
    ...
    for cv_split, (tr, te) in enumerate(kfold.split(sleap_paths)):
        ...
        save_dir = os.path.join(project_dir, f'cv{cv_split}')
        os.makedirs(save_dir, exist_ok=False)

        # train
        model_name = train(train_paths, project_dir, save_dir, use_instance)
        print("model_name:", model_name)

        # test
        train_checkpoint_path = os.path.join(save_dir, model_name, 'checkpoint.p')
        test_log_y_and_model, test_log_ll = get_test_probs(test_paths, train_checkpoint_path,
                                                           project_dir, save_dir, use_instance)
