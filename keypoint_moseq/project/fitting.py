    ...
    for iteration in tqdm.trange(start_iter, num_iters+1):
        ...
        if (plot_every_n_iters>0 and (iteration%plot_every_n_iters)==0) or (iteration==num_iters):
            plot_progress(model, data, history, iteration, name=name,
                          savefig=save_progress_figs, save_dir=save_dir)

        if (save_every_n_iters>0 and (iteration%save_every_n_iters)==0) or (iteration==num_iters):
            save_checkpoint(model, data, history, batch_info, batch, iteration, name=name,
                            save_dir=save_dir,save_history=save_history,
...
def apply_model(*, params, coordinates, confidences=None,
                ...
                save_dir=None, name=None, results_path=None,
    ...
    if save_results:
        if results_path is None: 
            assert save_dir is not None and name is not None, fill(
                ...
                'or the ``save_dir`` and ``name`` arguments')
            results_path = os.path.join(save_dir,name,'results.h5')
    ...
    return results_dict, model, data
